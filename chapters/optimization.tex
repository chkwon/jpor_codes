...
\end{code}
\noindent Note that \kode{f\_line} is indeed a function of \kode{lambda}, while \kode{x\_bar} and \kode{d\_bar} are regarded as constant parameters, or optional arguments for the purpose of optimization with respect to \kode{lambda}. 

To create an ad-hoc function that depends on the current values of \kode{x\_bar} and \kode{d\_bar} and accepts \kode{lambda} as the only argument, we use 
%= lang: julia
\begin{code}
lambda -> f_line(x_bar, d_bar, lambda)
\end{code}
\noindent We now optimize \kode{f\_line} in the interval \kode{[0.0, 1.0]} using the Golden Section line search algorithm:
%= lang: julia
\begin{code}
using Optim
